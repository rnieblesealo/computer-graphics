...
def printMaterialInfo(material):
    ...
    if material["TEXTURES"]:
        print("diffuse_tex: ", material["TEXTURES"]
              [assimp_py.TextureType_DIFFUSE])
...
def getObjectDataList(filename, verbose=False):
    ...
    for mesh in scene.meshes:
        texNames.append(
            folderPath.name+"/"+scene.materials[mesh.material_index]["TEXTURES"][assimp_py.TextureType_DIFFUSE][0])
        ...
        if nTextures == 0:
            ...
            print(
                nTextures, " different sets of Texture coordinates. Only the first one is loaded.")
        vertices = np.asarray(mesh.vertices, dtype="f4").reshape(-1, 3)
        verticesList.append(vertices)
        geomData = np.concatenate((vertices, np.asarray(
            mesh.texcoords[0], dtype="f4").reshape(-1, 2)), axis=1).astype("float32").flatten()
